...
{
    int remaining_length = 2 + 2 + topic_size + payload_size; // Tamanho correto do pacote
    int fixed_header_length = 2;
    ...
    this->buffer[0] = 0x30; // MQTT PUBLISH header (QoS 0, no retain)
    ...
    int msg_id = 10;
    this->buffer[fixed_header_length + 2 + topic_size] = (msg_id >> 8) & 0xFF; // Topic Length MSB
    this->buffer[fixed_header_length + 2 + topic_size + 1] = msg_id & 0xFF;    // Topic Length LSB

    memcpy(&(buffer[fixed_header_length + topic_size + 4]), payload, payload_size); // Copia o payload
